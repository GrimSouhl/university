    ...
    public Objeto generarCodigoInstancia(Instancia instancia, String metodo, Objeto[] params, int linea)
        ...
        switch (metodo) {
            ...
            case Metodo.XOR:
                ...
                var = new Variable(newNombObj(), instancia.getBloque(), false, this);
                checktrue = newEtiqueta();
                ini = newEtiqueta();
                fin = newEtiqueta();
                terminar = newEtiqueta();

                //SOLO ES VERDADERO SI UNA DE LAS OPERACIONES DE LAS QUE OPERA ES VERDADERA Y LA OTRA FALSA
                PLXC.out.println("if(" + instancia.getIDC() + "== 0 ) goto " + checktrue + ";");
                //si es uno y el otro es uno va a false
                PLXC.out.println("if( " + varq.getIDC() + " == 1 ) goto " + fin + ";"); //los dos son 1, false
                //si es 1 y 0 va a true
                PLXC.out.println(" goto " + ini); 
        
                //checktrue
                PLXC.out.println(checktrue + " :"); //ETTRUE
                PLXC.out.println("if( " + varq.getIDC() + " == 0 ) goto " + fin + ";"); //los dos son false,false
                //true:
                PLXC.out.println(ini + " :"); //ETTRUE
                PLXC.out.println("	" + var.getIDC() + " = 1 ;"); //VERDADERO
                PLXC.out.println(" goto " + terminar);
                //false:
                PLXC.out.println(fin + " :"); //ETFALSE
                PLXC.out.println("	" + var.getIDC() + " = 0 ;");
                //terminar:
                PLXC.out.println(terminar + " :");
                return var;
