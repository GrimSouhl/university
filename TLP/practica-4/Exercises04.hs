...
-- | Exercise 1.1 - Update the variable environment and the store

-- | 'NaturalSemantics.hs' contains definitions for locations, stores,
-- | and variable environments.
-- |
-- | Complete the definitions of the functions 'updateVarEnv' and
-- | 'updateStore' in 'NaturalSemantics.hs'.

-- | Exercise 1.2 - Natural semantics for variable declarations

-- | 'NaturalSemantics.hs' defines the algebraic data type 'ConfigD' to
-- | represent configurations of the judgements ->_D for variable
-- | declarations using locations.

-- | Modify 'NaturalSemantics.hs' and complete the definition of the
-- | function 'nsDecV', which implements the judgements ->_D.
...
-- Note that accessing a non-allocated location raises a runtime error.
initStore :: Store
initStore l
  | l == next = 1
  | otherwise = error $ "undefined location " ++ show l

-- | Then, we declare some local variables:

declarations :: DecVar
declarations =  Dec "x" (N "5")                   -- var x:= 5;
               (Dec "y" (N "2")                   -- var y:= 2;
               (Dec "z" (Mul (V "x") (V "y"))     -- var z:= x * y;
               (Dec "x" (Add (V "x") (N "1"))     -- var x:= x + 1;
                EndDec)))

-- | Then, define a function 'showDecV' that shows the variables declared in a
-- | 'DecVar'. For each variable 'v' in the list 'vars', it shows
-- | both its location and its value:

showDecV :: DecVar -> EnvVar -> Store -> [Var] -> String
showDecV decs env sto vars = foldr (showVar env' sto') [] vars ++ showNext
  where
     FinalD env' sto' = nsDecV (InterD decs env sto)
     showVar env sto x s = "var " ++ x ++ " loc " ++ show (env' x) ++ " val " ++ show (sto' . env' $ x) ++ "\n" ++ s
     showNext = "next -> " ++ show (sto' next) ++ "\n"
...
-- | Exercise 2.1 - Update the procedure environment
-- |
...
-- | 'envProc envP p' looks up a definition of procedure 'p' in  the
-- | procedure environment 'envP'. If 'p' is defined in 'envP', then
-- | 'envProc' returns a tuple (s, envV', envP'), where 's' is the body of 'p',
-- | and 'envV'' and 'envP'' are the snapshots of the variable and procedure
-- | environments associated with 'p'.  If 'p' is not defined in 'envP',
-- | the function raises an error "undefined procedure p".
...
-- | The, we define the function 'showDecP', which shows the procedures declared in
...
-- | Finally, we can implement the natural semantics of WHILE.
-- | 'NaturalSemantics.hs' defines the algebraic data type
-- | 'Config', whicH represents the configurations of the judgements.

-- | Complete the definition of the judgements in 'nsStm'.
-- | Note that there are two rules for 'call p', depending on whether
-- | WHILE allows recursive procedures or not. Implement only the rule
...
-- | relate memory cells to variables by numbering the variables starting from 1.
...
-- | and 'RecursiveFactorial.w' in the directory 'Examples' to test your
-- | implementation of the Natural Semantics. For example:
-- |
