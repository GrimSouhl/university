...
isEmpty (SC mempool hist epoch)= S.isEmpty mempool && D.isEmpty hist && epoch==0 
...
size (SC _ hist _) = sum ( map (S.size . snd) (D.keysValues hist))
...
add transaccion (SC mempool hist epoch) = SC (S.insert transaccion mempool) hist epoch
...
validate sc@(SC mempool hist epoch)
  | S.isEmpty mempool = sc
  ---any bool lista: aplica la funcion a toda la lista
  ---de la lista sacamos tx(transaccion) y miramos si esta validada
  |any (\tx -> getEpoch tx sc /= -1) (S.elements mempool) = error "transaccion ya validada"
...
addAll (t:ts) (SC mempool hist epoch) = (SC (agregar (t:ts) mempool) hist epoch)
  where 
    agregar [ ] mempool = mempool
    agregar (t:ts) mempool =agregar ts (S.insert t mempool) 
...
fold funcion valorIni sc@(SC mempool hist epoch) = 
  D.foldKeysValues (\_ set acc -> S.fold funcion acc set) valorIni hist
