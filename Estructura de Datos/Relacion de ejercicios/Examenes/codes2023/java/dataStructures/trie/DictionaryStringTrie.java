...
public class DictionaryStringTrie<V> {
  ...
  ****************************************************************************/
  ...
  // | = Exercise d - size
  /**
   * Implementa el método size que devuelve el número de cadenas almacenadas el Trie.
   * Observa que por cada cadena almacenada habrá un nodo final en el Trie.
   * Por ejemplo, para el Trie de la figura 1, el método size tiene que devolver 6.
   */
  public int size() {
    return size(root);
  }
  ...
  protected static <V> int size(Node<V> node) {
    if( node == null){
      ...
      int total = sizeValue(node.value);
  ...
  protected static <V> V search(String str, Node<V> node) {
    if(node == null){
      ...
      if(str.isEmpty()){
        ...
        Node<V> child = childOf(first, node);
        return search(rest, child);
      }
    }
  }
  ...
  protected static <V> Node<V> insert(String str, V value, Node<V> node) {
    ...
    Node<V> child = childOf(first, node);
    Node<V> newChild = insert(rest, value, child);
    node.children.insert(first, newChild);
    return node;
  }
